...
public class LoanController {
	...
	@PostMapping("/{userId}")
	@PreAuthorize("hasRole('ROLE_ADMIN') or @authorizationService.hasAccess(authentication, #userId)")
	public ResponseEntity<Void> createLoan(@PathVariable UUID userId, @RequestBody CreateLoanRequest createLoanRequest) {
		loanService.createLoan(createLoanRequest, userId);
		return ResponseEntity.ok().build();
	}
}
